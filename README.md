...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .example-word { color: var(--primary); font-weight: 600; cursor: pointer; border-bottom: 1px dashed var(--primary-light); transition: all 0.2s; }
        .example-word:hover { color: var(--accent); border-bottom-color: var(--accent); }
    ...
    <div class="app-container">
        <header class="header">
            <div style="display: flex; align-items: center; justify-content: center; gap: 16px; margin-bottom: 12px;">
                <div style="width: 56px; height: 56px; background: linear-gradient(135deg, var(--primary), var(--accent)); border-radius: 14px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(30,58,95,0.2);">
...
function renderResult(data) {
    ...
    for (let meaning of data.meanings) {
        ...
        for (let i = 0; i < defs.length; i++) {
            ...
            let example = '';
            if (def.example) {
                // Split the example into words and make each clickable
                let words = def.example.split(/(\s+)/);
                let processedWords = words.map(function(w) {
                    // Skip whitespace
                    if (/^\s+$/.test(w)) return w;
                    // Skip punctuation-only tokens but keep them
                    if (/^[^a-zA-Z]+$/.test(w)) return w;
                    // Clean the word for searching (remove trailing punctuation)
                    let cleanWord = w.replace(/[^a-zA-Z]+$/, '');
                    let trailing = w.substring(cleanWord.length);
                    if (cleanWord.length > 0) {
                        return '<span class="example-word" onclick="performSearch(' + "'" + cleanWord.toLowerCase() + "'" + ')" title="Click to search">' + cleanWord + '</span>' + trailing;
                    }
                    return w;
                });
                example = processedWords.join('');
            }
